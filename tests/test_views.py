...
def test_short_url(patch_user_agent, django_db_setup):
    ...
    assert response.status_code == 302 and response.url == dh.original_url

@pytest.mark.django_db
def test_handler404(django_db_setup, patch_user_agent, monkeypatch):
    monkeypatch.setattr('heyurl.views.render', lambda request, template: mocks.http_request())
    response = views.handler404(mocks.http_request(), None)
    assert response.url == dh.original_url

@pytest.mark.django_db
def test_month_metrics(django_db_setup):
    response = views.month_metrics(mocks.http_request())
    assert response.status_code == 200


@pytest.mark.django_db
def test_top_ten(django_db_setup):
    response = views.top_ten(mocks.http_request())
    assert response.status_code == 200
